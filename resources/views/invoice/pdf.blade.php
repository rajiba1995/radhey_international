...
<div class="invoice-details">
    ...
    <p><strong>Slip No:</strong> {{ $invoice->packing->slipno }}</p>
        ...
        <tbody>
            @foreach($invoice->order->items as $item)
                <tr>
                    ...
                    <td>{{ number_format($invoice->net_price/$item->quantity, 2) }}</td>
                    <td>{{ $item->quantity }}</td>
    ...
    <p><strong>Paid Amount:</strong> {{ number_format($invoice->net_price-$invoice->required_payment_amount, 2) }}</p>
